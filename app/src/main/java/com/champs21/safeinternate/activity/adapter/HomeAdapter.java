...
public class HomeAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private ArrayList<SafeInternetNews> mValues;
    private Context mContext;
    protected ItemListener mListener;
    private static final int HERO = 2;
    private static final int ITEM = 0;

    public HomeAdapter(Context context, ArrayList<SafeInternetNews> values, ItemListener itemListener) {
        mValues = values;
        mContext = context;
...
//        return new ViewHolder(view);
        ...
        switch (viewType) {
            case ITEM:
                View viewItem = inflater.inflate(R.layout.safe_internet_news_row, parent, false);
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder viewHolder, int position) {
        SafeInternetNews result = mValues.get(position);
        switch (getItemViewType(position)) {
            case HERO:
                final HeroVH topItem = (HeroVH) viewHolder;
//                topItem.title.setText(result.getDate());
                break;
            case ITEM:
